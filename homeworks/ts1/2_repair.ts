...
if (data.type == 'Money' && 'amount' in data.value ) {
    return data.value.amount;
  }
  else {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const unhandledType = data.type;
      const assertNever: never = unhandledType;
      // let unhandled: never = data; // здесь, возможно, нужно использовать нечто другое. :never должен остаться
